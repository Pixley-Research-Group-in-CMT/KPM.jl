...
scatter!(ωsreno, real.(res[:,1])./Da, label=L"\mathrm{Re}\;G_{xx}^{(z)}",markerstrokewidth=0.0)
scatter!(ωsreno, real.(res[:,2])./Da, label=L"\mathrm{Re}\;G_{yy}^{(z)}",markerstrokewidth=0.0)
